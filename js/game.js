...
function goToNextLevel(n) {
    ...
    overlay.querySelector(".sc-world").textContent = `${WORLD_NAMES[n - 1]} : ${LEVEL_NAMES[n - 1]}`
...
function showVictory() {
    ...
    document.getElementById("victory-score").textContent = String(gameState.totalScore).padStart(6, "0")
...
// Esta función esta pensada para el entorno de evaluación ya que permite saltarse los niveles sin haber completado el anterior.
window.goToLevel = function(n) {
    if (n >= 1 && n <= 5) showLevel(n)
}

// Esta función esta pensada para el entorno de evaluación ya que permite completar un nivel sin haberlo jugado.
document.addEventListener('DOMContentLoaded', () => {
  // Acceder al nivel desde la URL directa. Por ejemplo: La URL index.html?nivel=3,  mostrará el nivel 3 de forma directa.
